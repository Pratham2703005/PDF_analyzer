...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Check if OpenAI API key is configured when using OpenAI model
    if (model === "openai" && !process.env.OPENAI_API_KEY) {
      return NextResponse.json(
        {
          success: false,
          message:
            "OpenAI API key is not configured. Please add OPENAI_API_KEY to your environment variables or switch to local model.",
          requiresApiKey: true,
        },
        { status: 400 },
      )
